...
3. Save the file as humana_csv_unformatted.csv and put it in the same folder you'll be running the python script from. (you can change the name of the file in the script without any issue)
4. Run the script. (python scorecard_formatter.py)
5a. If the script worked right, there will be no output and you'll have a new file in the same folder called humana_findings.csv
...
with open('humana_csv_unformatted.csv', 'r') as f:
    reader = csv.reader(f)
    csv_list = list(reader)

    with open('humana_findings.csv', 'w') as csvfile:
