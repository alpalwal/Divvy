...
resource "aws_iam_policy" "power-user-policy" {
  name        = "divvycloud-power-user-policy"
  description = ""
  policy      = "${file("power_user_policy.json")}"
}
...
resource "aws_iam_policy_attachment" "attach-diivvycloud-write-policy" {
  name       = "Attach-write"
  roles      = ["${aws_iam_role.divvycloud.name}"]
  policy_arn = "${aws_iam_policy.power-user-policy.arn}"
}
